...
token = 'ODMwOTQ0MTMyNzU5Mjg5ODU2.Yw.Xnck02ueFALNrWxWfxRYlAQG76o'
client = discord.Client()
client = commands.Bot(command_prefix=commands.when_mentioned_or('j!'), case_insensitive=True, intents=discord.Intents.all())
...
db = mongoClient.get_database("anitnuke").get_collection("servers")
db2 = mongoClient.get_database("Antinuke").get_collection("protection")
...
async def on_connect():
    ...
    watch = discord.Activity(type = discord.ActivityType.watching, name=f'j!help')
...
async def on_guild_join(guild):
    ...
    embed = discord.Embed(title='Antinuke', color=0xf14645, description=f'Joined New Server!')
    ...
    try:
        ...
        if punishment == 'kick':
            try:
                ...
                await guild.kick(discord.Object(logs.user.id), reason=f"Jose Protection System | {reason}")
        channel = db.find_one({ "guild_id": guild.id })['log']
        if channel == None:
            pass
        else:
            log_channel = client.get_channel(channel)
            embed = discord.Embed(title='JoseXAxis', color=0xf14645)
            embed.description = 'Antinuke Protection System'
            embed.set_thumbnail(url='https://cdn.discordapp.com/attachments/822279744263290930/822306117190811648/image0.gif')
...
async def on_guild_role_update(before, after):
    try:
        ...
        if punishment == 'ban':
            try:
                await logs.user.ban(reason=f"Antinuke Protection System | {reason}")
            except:
                await guild.ban(discord.Object(logs.user.id), reason=f"Antinuke Protection System | {reason}")
        if punishment == 'kick':
            try:
                await logs.user.kick(reason=f"Antinuke Protection System | {reason}")
            except:
                await guild.kick(discord.Object(logs.user.id), reason=f"Antinuke Protection System | {reason}")
        channel = db.find_one({ "guild_id": guild.id })['log']
        if channel == None:
            pass
        else:
            log_channel = client.get_channel(channel)
            embed = discord.Embed(title='Antinuke', color=0xf14645)
            embed.description = 'Antinuke Protection System'
            embed.set_thumbnail(url="")
            embed.add_field(name='Banned', value=f'**`{reason}`**',inline=False)
            embed.add_field(name='Reason', value=f'**`{reason}`**',inline=False)
            await log_channel.send(embed=embed)
    except:
        pass 
...
async def on_webhook_update(webhook):
    try:
        ...
        if channel == None:
            ...
            log_channel = client.get_channel(channel)
            embed = discord.Embed(title='Jose', color=0xf14645)
...
async def on_member_kick(guild, member):
    try:
        ...
        if channel == None:
            ...
            log_channel = client.get_channel(channel)
            embed = discord.Embed(title='Jose', color=0xf14645)
            embed.description = 'Jose Protection System'
            embed.set_thumbnail(url='https://cdn.discordapp.com/icons/795579014122438688/e2cb59f60423ffd8f561f40827400fe3.webp?size=1024')
            embed.add_field(name='Banned', value=f'**`{logs.user}`**', inline=False)
            embed.add_field(name='Reason', value=f'**`{reason}`**', inline=False)
            await log_channel.send(embed=embed)
...
async def botinfo(ctx):
    ...
    embed.add_field(name='Name', value='`JoseXAxis`', inline=False)
    ...
    embed.add_field(name='Creators', value=f'`SexyMadaFuka`', inline=False)
    embed.set_thumbnail(url='https://cdn.discordapp.com/icons/795579014122438688/e2cb59f60423ffd8f561f40827400fe3.webp?size=1024')
...
async def invite(ctx):
    embed = discord.Embed(title='Jòse', color=0xf14645, description=f'[Invite Jòse](https://discord.com/api/oauth2/authorize?client_id=809916885873197057&permissions=8&scope=bot)')
    embed.set_thumbnail(url='https://cdn.discordapp.com/icons/795579014122438688/e2cb59f60423ffd8f561f40827400fe3.webp?size=1024')
    await ctx.send(embed=embed)
...
async def setup(ctx):
    ...
    embed.add_field(name='Anti-Nuke', value=f'To Enable Jose\'s Protection Make Sure The `{client.user.name}` Role Is As High As Possible, Anyone Above The `{client.user.name}` Role Will Bypass Our Protection. Otherwise This Will Protect Your Server From Being Nuked, Wizzed, Destroyed The Anti-Nuke Module Is Automaticly Enabled, If You Wish To Turn Some Of Them Off Type `a!toggle [MODULE]` To View All The Modules Type `j!settings` I Hope You Enjoy.', inline=False)
...
async def unban(ctx, member_id = None):
    if member_id == None:
        ...
        embed = discord.Embed(title='Jòse', color=0xf14645, description=f'**`Successfully Unbanned {member_id}!`**')
        embed.set_thumbnail(url='https://cdn.discordapp.com/icons/795579014122438688/e2cb59f60423ffd8f561f40827400fe3.webp?size=1024')
        await ctx.send(embed=embed)
@client.command()
async def guild(ctx):
  e = discord.Embed(color=0xf14645)
  e.add_field(name="guilds",value=', '.join(s.name for s in client.guilds), inline=False)
  await ctx.send(embed=e)
client.run("ODMwOTQ0MTMyNzU5Mjg5ODU2.YHw.Xnck02ueFALNrWxWfxRYlAQG76o")
